...
## 20. TSS (m-of-n threshold) wallets

Vectors: `app/core/tests/vectors/tss-v1.json`, verified by
...
key generation is a DKLs23 DKG among `n` parties, signing is a threshold
...
- the 65-byte `r ‖ s ‖ (27+v)` serialization and high-s rejection (§4.2);
- the challenge → sign → login flow (§4.3) — the server cannot tell;
- transaction assembly: `v = chainId·2 + 35 + recoveryId`, leading-zero
  stripping, RLP (§16), starting from the ceremony's `(r, s, recoveryId)`.

The one protocol-visible consequence: **threshold ECDSA is nondeterministic**
(the nonce is jointly random), so two ceremonies over the same message yield
different valid signatures. Therefore a TSS wallet must never use the
signature-derived E2EE key derivation of §5. Instead its encryption keypair
is generated at wallet creation, bound with one ceremony signature over the
§6 binding message (any valid signature binds — determinism is not needed
there), and stored under the wallet's custody. Login publishes that stored
`publicKey`/`publicKeySig` like any other client.

### 20.1 The recorded vector

`tss-v1.json` records one real 2-of-3 DKLs23 ceremony (parties
`{0, 2}` signing — a strict subset, which is the m-of-n property itself).
Because the nonce is jointly random the *signing* side cannot be
regenerated byte-for-byte; what a port must reproduce is the
**verification** side, using only its ordinary single-key code:

1. `recoverAddress(message, messageSignature)` (§4) equals `address`;
2. `messageSignature` is low-s, `v ∈ {27, 28}`;
3. rebuilding the §16 transaction from the JSON fields reproduces
   `tx.sighash`; assembling with the recorded `(r, s, recoveryId)`
   reproduces `tx.raw` and `tx.hash` byte-for-byte;
4. recovering the signer of `tx.sighash` from `(r, s, recoveryId)` yields
   `address`.

A fresh vector (a new wallet and new signatures, equally valid) comes from
