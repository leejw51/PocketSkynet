...
test.describe("mentions", () => {
  test("an @name in plaintext reaches the inbox and clears when the room is read", async ({
    ...
    const sent = await post(
      ...
      alice,
      room,
      `morning @${bob.username}, can you look at this?`,
    );
    ...
    await api(request, bob).post(`/api/rooms/${room}/read`, {
      lastReadSerial: sent.msgSerial,
    });
  ...
  test("punctuation and email addresses do not manufacture mentions", async ({
    request,
  }) => {
    ...
    const before = (await json(await api(request, bob).get("/api/mentions")))
      .length;
    ...
    await post(
      ...
      alice,
      room,
      "write to bob@example.com, cost @ 5 dollars",
    );
    expect(
      (await json(await api(request, bob).get("/api/mentions"))).length,
    ).toBe(before);
  ...
  test("an encrypted message mentions through the declared list only", async ({
    request,
  }) => {
  ...
  test("a mention list is validated, not trusted", async ({ request }) => {
    ...
    const malformed = await api(request, alice).post(
    ...
    const tooMany = await api(request, alice).post(
      `/api/rooms/${room}/messages`,
      {
        content: "hi",
        msgHash: hash("a"),
  ...
  test("editing a mention away removes it, and editing one in adds it", async ({
    request,
  }) => {
  ...
  test("a mention in a thread reply still reaches the inbox", async ({
    request,
  }) => {
  ...
  test("leaving a room takes its mentions out of the inbox", async ({
    request,
  }) => {
    ...
    const room = (
      await json(
        await api(request, alice).post("/api/rooms", { name: "Temporary" }),
      )
    ).id;
