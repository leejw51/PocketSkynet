...
test.describe("on a phone", () => {
  test("sending scrolls to the newest message", async ({ page, request }) => {
    ...
    await expect
      .poll(distanceFromBottom, { timeout: 20_000 })
      .toBeLessThan(await slack());
  ...
  test("stays at the bottom when a media row grows after it loads", async ({
    page,
    request,
  }) => {
    ...
    await expect
      .poll(async () => (await geo()).d, { timeout: 20_000 })
      .toBeLessThan(slack);
  ...
  test("a message arriving while pinned still follows on a small screen", async ({
    ...
    await expect(stream.getByText("landed on the phone")).toBeVisible({
      timeout: 20_000,
    });
    await expect
      .poll(distanceFromBottom, { timeout: 15_000 })
