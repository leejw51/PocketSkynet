...
test.describe("server administration", () => {
  test("the login response and /admin/session agree on who is an admin", async ({
    request,
  }) => {
    ...
    expect(
      (await json(await api(request, boss).get("/api/admin/session")))
        .isServerAdmin,
    ).toBe(true);
  ...
  test("every admin route refuses a non-admin", async ({ request }) => {
    ...
    expect(
      (
        await api(request, alice).post(
          `/api/admin/users/${alice.address}/suspend`,
        )
      ).status(),
    ).toBe(403);
    expect(
      (await api(request, alice).delete("/api/admin/rooms/room_1_x")).status(),
    ).toBe(403);

    // And an unauthenticated caller gets 401, not a hint that the route exists.
    expect((await api(request, null).get("/api/admin/overview")).status()).toBe(
      401,
    );
  });
  ...
  test("the overview counts the server and echoes the configured admin list", async ({
    ...
    await api(request, alice).post("/api/rooms/dm", {
      walletAddress: boss.address,
    });

    const overview = await json(
      await api(request, boss).get("/api/admin/overview"),
    );
  ...
  test("suspending invalidates a token that was already issued", async ({
    request,
  }) => {
    const boss = await signIn(request, "boss");
    ...
    await expect(
      signIn(request, "ad-susp-carol", { fresh: true }),
    ).rejects.toThrow(/403/);
  ...
  test("an admin cannot suspend or remove themselves", async ({ request }) => {
    ...
    expect(
      (
        await api(request, boss).post(
          `/api/admin/users/${boss.address}/suspend`,
        )
      ).status(),
    ).toBe(400);
    expect(
      (
        await api(request, boss).delete(`/api/admin/users/${boss.address}`)
      ).status(),
    ).toBe(400);
    // Still working afterwards — no half-applied state.
    expect((await api(request, boss).get("/api/admin/overview")).status()).toBe(
      200,
    );
  });

  test("removing someone evicts them everywhere and flags a re-key", async ({
    request,
  }) => {
    const boss = await signIn(request, "boss");
    ...
    const room = (
      await json(
        await api(request, alice).post("/api/rooms", { name: "Payroll" }),
      )
    ).id;
    await api(request, alice).post(`/api/rooms/${room}/invite`, {
    ...
    const removed = await api(request, boss).delete(
      `/api/admin/users/${carol.address}`,
    );
  ...
  test("an admin can manage and delete a room they were never in", async ({
    request,
  }) => {
    const boss = await signIn(request, "boss");
    ...
    expect(
      (await api(request, boss).delete(`/api/admin/rooms/${room}`)).status(),
    ).toBe(404);
  });
  ...
  test("purging a room history is admin-only", async ({ request }) => {
    ...
    const room = (
      await json(
        await api(request, alice).post("/api/rooms", { name: "History" }),
      )
    ).id;
    await api(request, alice).post(`/api/rooms/${room}/invite`, {
    ...
    const refused = await api(request, bob).delete(
      `/api/rooms/${room}/messages`,
    );
    expect(refused.status()).toBe(403);
    expect((await refused.json()).message).toContain("Only room admins");
    expect(
      (await api(request, bob).delete(`/api/messages/${message.id}`)).status(),
    ).toBe(200);

    // ...but erasing everybody's record in one request is the admin's.
    expect(
      (
        await api(request, alice).delete(`/api/rooms/${room}/messages`)
      ).status(),
    ).toBe(200);
    expect(
      (await api(request, boss).delete(`/api/rooms/${room}/messages`)).status(),
    ).toBe(200);
  });
});
