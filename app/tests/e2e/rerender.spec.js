...
const survivors = (page, selector) =>
  page.evaluate((sel) => {
    ...
    document
      ...
      .forEach((r) => (r.dataset.marked ? kept++ : fresh++));
    return { kept, fresh };
  }, selector);
...
test.describe("re-rendering", () => {
  test("a new message adds one row and keeps the rest", async ({
    page,
    request,
  }) => {
    ...
    expect(await survivors(page, ".fn-stream article")).toEqual({
      kept: 12,
      fresh: 1,
    });
  });

  test("sending from the composer keeps the rows already on screen", async ({
    page,
    request,
  }) => {
    ...
    await expect(rows).toHaveCount(13, { timeout: 20_000 });
    await expect
      .poll(async () => (await survivors(page, ".fn-stream article")).kept, {
        timeout: 15_000,
      })
      .toBe(12);
  });
  ...
  test("a room list edit keeps the room rows", async ({ page, request }) => {
    ...
    const rooms = [];
    for (let i = 0; i < 5; i++)
      rooms.push(await channel(request, me, `Sidebar ${i}`));
