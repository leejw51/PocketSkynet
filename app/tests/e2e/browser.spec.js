...
test.describe("the web client", () => {
  ...
  test("renders a channel whose messages carry the new thread fields", async ({
    ...
    const root = await post(
      ...
      alice,
      room,
      "does the client still render me?",
    );
    await post(request, alice, room, "a threaded reply", {
  ...
  test("renders a DM and its mention badge", async ({ page, request }) => {
    ...
    await post(
      ...
      bob,
      room,
      `morning @${alice.username}, ready when you are`,
    );
    ...
    await expect(page.getByText("Direct message", { exact: true })).toHaveCount(
      0,
    );
    await page.getByRole("option", { name: new RegExp(bob.username) }).click();
    await expect(
      page.getByRole("log").getByText(/ready when you are/),
    ).toBeVisible({
      timeout: 15_000,
    });
  ...
  test("replies in a thread from the composer, and highlights a mention", async ({
    ...
    const root = await post(
      ...
      bob,
      room,
      "who is picking up the migration?",
    );
    ...
    await expect(
      page.getByRole("log").getByText(/picking up the migration/),
    ).toBeVisible({
      timeout: 15_000,
    });
  ...
  test("completes an @mention from the composer", async ({ page, request }) => {
    ...
    await expect(
      page.getByRole("option", { name: new RegExp(alice.username) }),
    ).toHaveCount(0);
      ...
      .poll(
        async () => {
          const inbox = await json(
            await api(request, bob).get("/api/mentions"),
          );
  ...
  test("opens the mentions inbox and jumps to the room", async ({
    page,
    request,
  }) => {
    ...
    await entry.click();
    await expect(
      page.getByRole("textbox", { name: /^Message Roadmap/ }),
    ).toBeVisible({
      timeout: 15_000,
    });
  ...
  test("shows the admin console only to an administrator", async ({
    page,
    request,
  }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Server admin" }),
    ).toHaveCount(0);
    expect(errors, errors.join(" | ")).toHaveLength(0);
  });
  ...
  test("an administrator gets the console", async ({ page, request }) => {
    ...
    await expect(
      page.getByText(/Administrators come from VITE_FRUITNATION_ADMIN/),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("dialog").getByText("Procurement"),
    ).toBeVisible();
  ...
  test("follows new messages when pinned, and holds still when reading back", async ({
    ...
    await expect(stream.getByText("backlog line 39")).toBeVisible({
      timeout: 15_000,
    });
    ...
    await expect
      .poll(distanceFromBottom, { timeout: 10_000 })
      .toBeLessThan(120);

    // A new message arrives while pinned: it follows.
    await post(request, bob, room, "arrived while pinned");
    await expect(stream.getByText("arrived while pinned")).toBeVisible({
      timeout: 15_000,
    });
    await expect
      .poll(distanceFromBottom, { timeout: 10_000 })
    ...
    await expect
      .poll(distanceFromBottom, { timeout: 5_000 })
      .toBeGreaterThan(400);
  ...
  test("sending your own message scrolls you to it, even from up the history", async ({
    ...
    await expect(page.locator(".fn-stream article")).toHaveCount(40, {
      timeout: 30_000,
    });
    const distanceFromBottom = () =>
      stream.evaluate((el) => el.scrollHeight - el.scrollTop - el.clientHeight);
    ...
    await expect
      .poll(distanceFromBottom, { timeout: 15_000 })
      .toBeLessThan(120);
  ...
  test("holds the newest message through a cold open, threads and growing media", async ({
    ...
    for (let i = 15; i < 25; i++)
      await post(request, bob, room, `message ${i}`);
    ...
    await expect
      .poll(async () => (await geo()).d, { timeout: 25_000 })
    ...
    await expect
      .poll(async () => (await geo()).d, { timeout: 10_000 })
      .toBeLessThan(await slack());
    ...
    await expect
      .poll(async () => (await geo()).d, { timeout: 15_000 })
      .toBeLessThan(await slack());
  ...
  test("a reflow that moves the scroll does not count as reading history", async ({
    ...
    const dist = () =>
      stream.evaluate((el) => el.scrollHeight - el.scrollTop - el.clientHeight);
    const slack = await stream.evaluate((el) =>
      Math.max(120, el.clientHeight * 0.2),
    );
    ...
    await box.press("Enter");
    await expect(stream.getByText("after the reflow")).toBeVisible({
      timeout: 15_000,
    });
    await expect.poll(dist, { timeout: 15_000 }).toBeLessThan(slack);
    await expect(page.locator(".fn-jump-latest")).toHaveCount(0);
  ...
  test("a colleague lights up when they arrive and goes dark when they leave", async ({
    ...
    await expect(
      bobRow.locator(".fn-ident--online, .fn-ident--away"),
    ).toHaveCount(0);
    ...
    await expect(bobRow.locator(".fn-ident--online")).toHaveCount(1, {
      timeout: 15_000,
    });
    ...
    await expect(bobRow.locator(".fn-ident--online")).toHaveCount(0, {
      timeout: 20_000,
    });
  ...
  test("the members roster spells the status out beside the name", async ({
    page,
    request,
  }) => {
    ...
    const mine = page
      .locator(".fn-person", { hasText: alice.username })
      .first();
    ...
    const theirs = page
      .locator(".fn-person", { hasText: bob.username })
      .first();
    await expect(theirs.locator(".fn-presence")).toHaveCount(0);
    await expect(
      theirs.locator(".fn-ident--online, .fn-ident--away"),
    ).toHaveCount(0);
  ...
  test("reacts to a message inside a thread", async ({ page, request }) => {
    ...
    const replyRow = page
      .getByRole("article")
      .filter({ hasText: "the reply to react to" });
    ...
    await expect
      .poll(
        async () => {
          const agg = await json(
            await api(request, alice).get(
              `/api/messages/${reply.id}/emoticons`,
            ),
          );
    ...
    expect(
      rootAgg,
      "the root must not have picked up the reply's reaction",
    ).toHaveLength(0);

    // And it renders back on the reply.
    await expect(replyRow.locator(".fn-reactions")).toBeVisible({
      timeout: 15_000,
    });
  ...
  test("sending a message from the browser still works", async ({
    page,
    request,
  }) => {
    ...
    await expect(
      page.getByRole("log").getByText("typed in a real browser"),
    ).toBeVisible({
      timeout: 15_000,
    });
