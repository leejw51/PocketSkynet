...
test.describe("presence", () => {
  test("a beacon makes you visible to a room-mate and to nobody else", async ({
    request,
  }) => {
    ...
    for (const who of [alice, bob, stranger]) {
      const declared = await api(request, who).put("/api/presence", {
        status: "online",
      });
  ...
  test("away is a status a client may declare; offline is not", async ({
    request,
  }) => {
    ...
    const refused = await api(request, alice).put("/api/presence", {
      status: "offline",
    });
    expect(refused.status()).toBe(400);
    expect((await refused.json()).message).toContain("derived");

    const nonsense = await api(request, alice).put("/api/presence", {
      status: "in a meeting",
    });
  ...
  test("a block hides presence in both directions", async ({ request }) => {
    ...
    await api(request, alice).post("/api/users/block", {
      address: bob.address,
    });
  ...
  test("an open event stream is presence on its own, and closing it ends it", async ({
    ...
    const { ticket } = await json(
      await api(request, alice).post("/api/events/ticket"),
    );
