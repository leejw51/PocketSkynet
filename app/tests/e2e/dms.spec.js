...
test.describe('direct messages', () => {
  test('opening the same DM from either side lands in one room', async ({ request }) => {
  ...
  test('three people get a group DM, distinct from any pair', async ({ request }) => {
    ...
    const pair = await json(
      await api(request, alice).post('/api/rooms/dm', { walletAddress: carol.address }),
    );
  ...
  test('a DM refuses every verb that only makes sense for a channel', async ({ request }) => {
    ...
    const room = await json(
      await api(request, alice).post('/api/rooms/dm', { walletAddress: bob.address }),
    );
    ...
    expect((await a.patch(`/api/rooms/${room.id}`, { name: 'Renamed' })).status()).toBe(400);
    expect(
      (await a.post(`/api/rooms/${room.id}/invite`, { userAddress: carol.address })).status(),
    ).toBe(400);
    expect((await a.post(`/api/rooms/${room.id}/kick`, { userAddress: bob.address })).status())
      .toBe(400);
    expect((await a.post(`/api/rooms/${room.id}/admins`, { walletAddress: bob.address })).status())
      .toBe(400);
  ...
  test('a DM to a wallet that has never signed in is refused', async ({ request }) => {
  ...
  test('naming only yourself opens a private notebook', async ({ request }) => {
    ...
    const room = await json(
      await api(request, carol).post('/api/rooms/dm', { walletAddress: carol.address }),
    );
    expect(room.kind).toBe('dm');
    ...
    const listed = await json(await api(request, carol).get(`/api/rooms/${room.id}/messages`));
    expect(listed.map((m) => m.content)).toEqual(['remember to rotate the keys']);
  });
  ...
  test('a DM is private to its members', async ({ request }) => {
    ...
    expect((await api(request, outsider).get(`/api/rooms/${room.id}/messages`)).status()).toBe(403);
    expect((await api(request, outsider).get(`/api/rooms/${room.id}`)).status()).toBe(403);
  });

  test('an ordinary channel still reports itself as one', async ({ request }) => {
