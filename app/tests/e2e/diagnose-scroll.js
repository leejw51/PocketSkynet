...
(() => {
  ...
  if (!log)
    return console.log("PS-DIAG: no [role=log] on screen — open a room first");
  ...
  console.log(
    "PS-DIAG bundle :",
    wasm.join(", ") || "(not seen in this navigation)",
  );
  ...
  const docScrolls =
    ...
    document.scrollingElement.clientHeight + 4;
  console.log(
    "PS-DIAG doc scrolls?",
    docScrolls,
    "docScrollTop=",
  ...
  const timer = setInterval(() => {
    ...
    if (now !== last) {
      console.log("PS-DIAG change :", now);
      last = now;
    }
  }, 300);
