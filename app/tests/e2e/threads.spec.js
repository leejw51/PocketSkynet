...
test.describe('threads', () => {
  test('a thread costs the channel one line and reports its size', async ({ request }) => {
    ...
    for (const text of ['checking', 'two tests left', 'green']) {
      const reply = await post(request, alice, room, text, { parentMessageId: root.id });
    ...
    expect(listed.map((m) => m.content)).toEqual(['shipping today?', 'unrelated']);
    ...
    const thread = await json(await api(request, alice).get(`/api/messages/${root.id}/thread`));
  ...
  test('replies posted in one burst keep their order', async ({ request }) => {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      await post(request, alice, room, `reply ${i}`, { parentMessageId: root.id });
    }
  ...
  test('replying to a reply joins its thread rather than nesting', async ({ request }) => {
    const alice = await signIn(request, 'th-flat-alice');
    const room = await channel(request, alice, 'Flat');
    const root = await post(request, alice, room, 'root');
    const first = await post(request, alice, room, 'first', { parentMessageId: root.id });
    const second = await post(request, alice, room, 'second', { parentMessageId: first.id });
    ...
    const fromReply = await json(await api(request, alice).get(`/api/messages/${first.id}/thread`));
  ...
  test('a reply cannot cross into another room', async ({ request }) => {
    ...
    const crossed = await api(request, alice).post(`/api/rooms/${publicRoom}/messages`, {
      content: 'leaking',
      msgHash: hash('c'),
    ...
    const orphan = await api(request, alice).post(`/api/rooms/${publicRoom}/messages`, {
      content: 'orphan',
      msgHash: hash('d'),
  ...
  test('deleting the root keeps the thread together', async ({ request }) => {
    ...
    const root = await post(request, alice, room, 'root');
    await post(request, alice, room, 'still here', { parentMessageId: root.id });

    expect((await api(request, alice).delete(`/api/messages/${root.id}`)).status()).toBe(200);
  ...
  test('a thread is not a way into a room', async ({ request }) => {
    ...
    const response = await api(request, carol).get(`/api/messages/${root.id}/thread`);
  ...
  test('replies still travel through /sync so a client can fold them itself', async ({
    ...
    const root = await post(request, alice, room, 'root');
    const reply = await post(request, alice, room, 'reply', { parentMessageId: root.id });

    const synced = await json(await api(request, alice).get(`/api/rooms/${room}/sync?since=0`));
    const syncedReply = synced.find((m) => m.id === reply.id);
    expect(syncedReply, 'a reply must reach /sync or offline clients lose it').toBeDefined();
