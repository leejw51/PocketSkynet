...
async function main() {
  ...
  if (await en.isVisible().catch(() => false)) { await en.click(); await page.waitForTimeout(800); }
  ...
  await page.locator('.fn-composer input[type=file]').first().setInputFiles(VIDEO);
  let seen = false, gone = 0;
  for (let i = 0; i < 480; i++) {
    const vis = await page.locator('.fn-transfers').isVisible().catch(() => false);
    if (vis) { seen = true; gone = 0; }
  ...
  //    own player uses.
  const src = await page.locator('.fn-attach__play video').first().getAttribute('src');
  console.log('media src present:', !!src);
  const storm = await page.evaluate(async ({ src, n }) => {
    const statuses = {};
    for (let i = 0; i < n; i++) {
      try {
        const r = await fetch(src, { headers: { Range: `bytes=${i * 17}-${i * 17 + 63}` } });
  ...
  const toasts = await page.locator('.fn-toast, [class*="toast"]').allInnerTexts().catch(() => []);
  const throttled = toasts.filter(t => /too many/i.test(t));
  console.log('throttle toasts:', JSON.stringify(throttled));
  console.log('console errors:', JSON.stringify(errors.filter(e => !/429/.test(e)).slice(0, 5)));
  await page.screenshot({ path: 'storm-after.png' });

  const pass = thumb > 0 && (storm['206'] || 0) === STORM && posted > 0 && throttled.length === 0;
