...
async function main() {
  ...
  for (let i = 0; i < 800; i++) {
    ...
    if (await rail.isVisible().catch(() => false)) {
      ...
      if (!sawRailLogged.has(txt)) {
        console.log("RAIL:", txt);
        sawRailLogged.add(txt);
      }
  ...
  try {
    ...
    if (await filesBtn.isVisible().catch(() => false)) {
      ...
      if (await dl.isVisible().catch(() => false)) {
        ...
        console.log(
          got === expected ? "DOWNLOAD MATCHES ORIGINAL" : "DOWNLOAD MISMATCH",
        );
      } else {
  ...
  for (const c of calls)
    seen.set(
      c.replace(/offset=\d+/, "offset=N"),
      (seen.get(c.replace(/offset=\d+/, "offset=N")) || 0) + 1,
    );
