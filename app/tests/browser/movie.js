...
async function main() {
  ...
  let sawRail = false, goneFor = 0, maxPct = 0;
  for (let i = 0; i < 9600; i++) {
    const vis = await page.locator('.fn-transfers').isVisible().catch(() => false);
    if (vis) {
      sawRail = true; goneFor = 0;
      const t = (await page.locator('.fn-transfers').innerText().catch(() => '')).replace(/\s+/g, ' ');
      const m = t.match(/(\d+)%/);
  ...
  console.log(`THUMBNAIL cost: ${(posterBytes / 1024 / 1024).toFixed(1)} MB of a ${(size / 1024 / 1024).toFixed(0)} MB film`);
  ...
  const state = await page.evaluate(async () => {
    ...
    return { before, after: { t: v.currentTime, ready: v.readyState }, dur: v.duration };
  });
