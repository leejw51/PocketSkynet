...
async function main() {
  ...
  if (!broke) {
    console.log("NEVER REACHED THE BREAK POINT");
    process.exit(1);
  }

  const firstAppends = log.filter(
    (e) => e.phase === "first" && e.method === "PATCH",
  ).length;
  ...
  console.log(
    sawRail && goneFor >= 8
      ? "transfer finished"
      : "transfer did NOT finish in budget",
  );
  await page.waitForTimeout(4000);
  ...
  const newSessions = second.filter(
    (e) => e.method === "POST" && e.path === "/api/uploads",
  );
  ...
  console.log(
    `first attempt: ${firstAppends} chunks sent before the interruption`,
  );
  console.log(
    `second attempt: ${newSessions.length} new sessions, ${probes.length} status probes, ${appends.length} chunks`,
  );
  console.log(`second attempt first append offset = ${firstOffset}`);

  const pass = newSessions.length === 0 && probes.length > 0 && firstOffset > 0;
  ...
  try {
    ...
    const got = crypto
      .createHash("sha256")
      .update(fs.readFileSync("/tmp/ps-resumed.bin"))
      .digest("hex");
    console.log(
      got === expected
