...
test("transport selection: default is fetch (http1), --http3 is curl", () => {
  ...
  const h3 = createTransport({
    baseUrl: "https://127.0.0.1:9101",
    http3: true,
  });
...
test("base URL is validated and trailing slashes are trimmed", () => {
  assert.throws(
    () => createTransport({ baseUrl: "not a url" }),
    TransportError,
  );
...
test("http3 with an http:// URL fails fast (QUIC mandates TLS)", () => {
  assert.throws(
    () =>
      buildCurlArgs("http://127.0.0.1:9099", {
        method: "GET",
        path: "/api/health",
      }),
    /https/,
...
test("curl argv: exact shape, URL last after --, secrets NOT in argv", () => {
  ...
  assert.ok(
    ...
    "body must not be an argv element",
  );
  ...
  for (const arg of args) {
    ...
    assert.ok(
      ...
      `auth header leaked into argv: ${arg}`,
    );
...
test("curl config: hostile body is one opaque, escaped config value", () => {
  const hostile = {
    content:
      "$(rm -rf /) `touch /tmp/pwned` ; & | > /etc/passwd '\" \\ \n --insecure",
  ...
  for (const arg of args) {
    assert.ok(
      !/[$`|;&<>]/.test(arg),
      `unexpected metacharacter in argv element: ${arg}`,
    );
  }
  assert.ok(
    !args.includes("--insecure"),
    "'--insecure' in the body must not add a flag",
  );
...
test("buildCurlConfig: token and body only, header-splitting refused", () => {
  ...
  const withToken = buildCurlConfig({
    method: "GET",
    path: "/api/rooms",
    token: "jwt.tok.en",
  });
...
test("request paths with whitespace or control chars are refused", () => {
  ...
  assert.throws(
    () => buildCurlArgs("https://h:1", { method: "GET", path: "api/health" }),
    TransportError,
  );
  assert.throws(
...
test("curl output parsing: status marker split", () => {
  ...
  assert.throws(
    () => parseCurlOutput("x\n__pskynet_http_status__:banana"),
    /unparseable/,
  );
});
