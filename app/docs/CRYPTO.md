...
| CSPRNG | **`getrandom`** (`0.2`) | See the wasm note below. Implemented as `core/src/random.rs` — one module, no fallback, an error on refusal — and deliberately *not* via `rand`'s `OsRng`, which wraps this and panics instead of reporting failure. Everything unguessable on either target draws from there. |
