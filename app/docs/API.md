...
**There are no TSS endpoints.** That is the design, not an omission: every
ceremony of the m-of-n threshold wallet (`docs/CRYPTO.md` §15;
`PROTOCOL.md` §20) — key generation, the login-challenge signature, every
transaction signature — runs inside the web client's WebAssembly, in a
dedicated worker. Share files and the wallet passphrase never leave the
browser, in any deployment.

What the server sees of a TSS wallet is exactly what it sees of a mnemonic
wallet: `POST /api/auth/challenge` for its address, then
`POST /api/auth/login` carrying an ordinary EIP-191 signature (plus
`publicKey`/`publicKeySig` for the E2EE identity sealed in the share
files). By §15.1's indistinguishability requirement the server cannot tell
which kind of wallet signed — there is deliberately nothing TSS-shaped to
implement on the API side.

Earlier releases (≤ 1.0.2) ran ceremonies server-side behind
