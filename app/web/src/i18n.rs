...
strings! {
    ...
    // custody model: CGGMP24 is the peer-reviewed threshold-ECDSA protocol
    // (Canetti–Gennaro–Goldfeder–Makriyannis–Peled, 2024 revision), and
    // cggmp24 is the Linux Foundation Decentralized Trust implementation —
    // the same team's successor of their independently audited cggmp21,
    // pure Rust so ceremonies can run in the browser. Protocol and crate
    // names stay in Latin script in every language — they are proper names.
    (tss_library_note, "Runs in your browser on the CGGMP24 threshold-ECDSA protocol via the cggmp24 library (Linux Foundation Decentralized Trust), successor of the audited cggmp21.", "브라우저 안에서 CGGMP24 임계값 ECDSA 프로토콜로 동작합니다. 감사를 받은 cggmp21의 후속인 cggmp24 라이브러리(Linux Foundation Decentralized Trust)를 사용합니다.", "ブラウザ内でCGGMP24しきい値ECDSAプロトコルを実行します。監査済みcggmp21の後継であるcggmp24ライブラリ（Linux Foundation Decentralized Trust）を使用しています。", "喺你嘅瀏覽器入面用 CGGMP24 門檻 ECDSA 協議執行，經由 cggmp24 程式庫（Linux Foundation Decentralized Trust）— 已審計 cggmp21 嘅後繼版本。", "Běží ve vašem prohlížeči na protokolu CGGMP24 pro prahové ECDSA prostřednictvím knihovny cggmp24 (Linux Foundation Decentralized Trust), nástupce auditované cggmp21.", "Se ejecuta en tu navegador con el protocolo CGGMP24 de ECDSA de umbral mediante la biblioteca cggmp24 (Linux Foundation Decentralized Trust), sucesora de la auditada cggmp21.", "在你的浏览器中基于 CGGMP24 门限 ECDSA 协议运行，采用 cggmp24 库（Linux Foundation Decentralized Trust），它是经审计的 cggmp21 的后继版本。", "Läuft in deinem Browser auf dem CGGMP24-Schwellwert-ECDSA-Protokoll über die cggmp24-Bibliothek (Linux Foundation Decentralized Trust), Nachfolgerin der auditierten cggmp21."),
    ...
    (tss_files_invalid, "{name} is not a TSS share file.", "{name}은(는) TSS 조각 파일이 아닙니다.", "{name}はTSSシェアファイルではありません。", "{name} 唔係 TSS 份額檔案。", "{name} není soubor s TSS podílem.", "{name} no es un archivo de parte TSS.", "{name} 不是 TSS 份额文件。", "{name} ist keine TSS-Anteilsdatei."),
    ...
    (tss_creating_hint, "Runs entirely in this browser — keep the page open; it can take several minutes.", "전부 이 브라우저 안에서 실행됩니다. 페이지를 열어 두세요. 몇 분 걸릴 수 있습니다.", "すべてこのブラウザ内で実行されます。ページを開いたままにしてください — 数分かかることがあります。", "全程喺呢個瀏覽器入面執行 — 唔好閂呢頁，可能要幾分鐘。", "Běží celé v tomto prohlížeči — nechte stránku otevřenou; může to trvat několik minut.", "Se ejecuta por completo en este navegador: mantén la página abierta; puede tardar varios minutos.", "完全在此浏览器中运行——请保持页面打开，可能需要几分钟。", "Läuft vollständig in diesem Browser — lass die Seite geöffnet; es kann einige Minuten dauern."),
