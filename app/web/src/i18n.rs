...
strings! {
    ...
    (tss_keygen_lost, "the server no longer reports this ceremony — it may have restarted. Run it again", "서버가 더 이상 이 의식을 알지 못합니다. 서버가 재시작되었을 수 있으니 다시 실행하세요", "サーバーがこのセレモニーを認識していません。再起動された可能性があります。もう一度実行してください", "伺服器已經唔記得呢個儀式 — 可能重啟過。請再執行一次", "server už tento ceremoniál nezná — možná se restartoval. Spusťte jej znovu", "el servidor ya no reconoce esta ceremonia; puede haberse reiniciado. Ejecútala de nuevo", "服务器已不再记录此仪式——可能已重启。请重新运行", "der Server kennt diese Zeremonie nicht mehr — er wurde womöglich neu gestartet. Führe sie erneut aus"),
    ...
    (tss_creating_hint, "Keep this page open — about a minute.", "이 페이지를 열어 두세요. 1분 정도 걸립니다.", "このページを開いたままにしてください — 1分ほどかかります。", "唔好閂呢頁 — 大約一分鐘。", "Nechte tuto stránku otevřenou — zabere to asi minutu.", "Mantén esta página abierta: tarda alrededor de un minuto.", "请保持此页面打开——大约需要一分钟。", "Lass diese Seite geöffnet — dauert etwa eine Minute."),
