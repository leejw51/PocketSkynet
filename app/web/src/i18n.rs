...
strings! {
    ...
    (server_address_hint, "Any PocketSkynet server, e.g. http://192.168.0.7:9099. It must allow this page's origin (PS_CORS_ORIGIN).", "모든 PocketSkynet 서버, 예: http://192.168.0.7:9099. 해당 서버가 이 페이지의 출처를 허용해야 합니다(PS_CORS_ORIGIN).", "任意の PocketSkynet サーバー。例: http://192.168.0.7:9099。このページのオリジンを許可している必要があります（PS_CORS_ORIGIN）。", "任何 PocketSkynet 伺服器，例如 http://192.168.0.7:9099。個伺服器要允許呢頁嘅來源（PS_CORS_ORIGIN）。", "Libovolný server PocketSkynet, např. http://192.168.0.7:9099. Musí povolovat původ této stránky (PS_CORS_ORIGIN).", "Cualquier servidor PocketSkynet, p. ej. http://192.168.0.7:9099. Debe permitir el origen de esta página (PS_CORS_ORIGIN).", "任意 PocketSkynet 服务器，例如 http://192.168.0.7:9099。该服务器必须允许此页面的来源（PS_CORS_ORIGIN）。", "Ein beliebiger PocketSkynet-Server, z. B. http://192.168.0.7:9099. Er muss den Ursprung dieser Seite erlauben (PS_CORS_ORIGIN)."),
