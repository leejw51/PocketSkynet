...
strings! {
    ...
    (attach_too_many, "You can attach up to {max} files at once — only the first {max} were added.", "한 번에 최대 {max}개까지 첨부할 수 있습니다 — 처음 {max}개만 추가되었습니다.", "一度に添付できるのは最大{max}件です — 最初の{max}件のみ追加されました。", "一次最多可附加 {max} 個檔案 — 僅新增了前 {max} 個。", "Najednou lze přiložit nejvýše {max} souborů — přidáno bylo jen prvních {max}.", "Puedes adjuntar hasta {max} archivos a la vez — solo se añadieron los primeros {max}.", "一次最多可附加 {max} 个文件 — 仅添加了前 {max} 个。", "Du kannst höchstens {max} Dateien auf einmal anhängen — nur die ersten {max} wurden hinzugefügt."),
    ...
    // --- MPC (m-of-n threshold) wallet — docs/CRYPTO.md §15 ----------------
    (tss_wallet_tab, "MPC wallet", "MPC 지갑", "MPCウォレット", "MPC 錢包", "MPC peněženka", "Cartera MPC", "MPC 钱包", "MPC-Wallet"),
    (tss_intro, "An m-of-n wallet: the private key never exists in one piece. You keep n sealed share files; any m of them sign by ceremony, and losing a few loses nothing.", "m-of-n 지갑입니다. 개인 키는 결코 한 조각으로 존재하지 않습니다. 봉인된 조각 파일 n개를 직접 보관하고, 아무 m개로 서명 의식을 수행하며, 몇 개를 잃어도 아무것도 잃지 않습니다.", "m-of-nウォレットです。秘密鍵は決して一つにまとまって存在しません。封印されたシェアファイルn個を自分で保管し、任意のm個でセレモニー署名します。数個失っても何も失われません。", "m-of-n 錢包：私鑰唔會完整咁存在。你自己保管 n 個封存咗嘅份額檔案，任意 m 個就簽到名，唔見咗幾個都唔會有損失。", "Peněženka m-z-n: soukromý klíč nikdy neexistuje vcelku. Sami držíte n zapečetěných souborů s podíly; libovolných m podepíše ceremoniálem a ztráta několika z nich nic neztrácí.", "Una cartera m-de-n: la clave privada nunca existe entera. Tú guardas n archivos de partes sellados; cualesquiera m firman por ceremonia, y perder unos pocos no pierde nada.", "m-of-n 钱包：私钥从不完整存在。你自己保管 n 个封存的份额文件，任意 m 个即可通过仪式签名，丢失几个也不会有任何损失。", "Eine m-von-n-Wallet: Der private Schlüssel existiert nie am Stück. Du bewahrst n versiegelte Anteilsdateien auf; beliebige m signieren per Zeremonie, und ein paar zu verlieren verliert nichts."),
    (tss_passphrase, "Passphrase", "암호 문구", "パスフレーズ", "密語", "Přístupová fráze", "Frase de contraseña", "口令", "Passphrase"),
    (tss_passphrase_hint, "Chosen at creation; needed for every signature. It never leaves this device.", "생성할 때 정한 문구로, 서명할 때마다 필요합니다. 이 기기를 절대 벗어나지 않습니다.", "作成時に決めたもので、署名のたびに必要です。この端末の外には出ません。", "建立錢包嗰陣設定，每次簽名都要用。永遠唔會離開呢部裝置。", "Zvolena při vytvoření; je potřeba pro každý podpis. Nikdy neopustí toto zařízení.", "Se elige al crearla y hace falta para cada firma. Nunca sale de este dispositivo.", "创建时设定，每次签名都需要。绝不会离开这台设备。", "Bei der Erstellung gewählt; für jede Signatur nötig. Sie verlässt dieses Gerät nie."),
    (tss_create_title, "Create an MPC wallet", "MPC 지갑 만들기", "MPCウォレットを作成", "建立 MPC 錢包", "Vytvořit MPC peněženku", "Crear una cartera MPC", "创建 MPC 钱包", "MPC-Wallet erstellen"),
    ...
    // Deliberately non-technical: the UI says "MPC wallet" and what the
    // custody model *means* — protocol and crate names live in
    // docs/CRYPTO.md §15 for whoever wants them, not on a login screen.
    (tss_library_note, "Everything runs in your browser — no server ever sees your shares or passphrase.", "모든 과정이 브라우저 안에서 실행됩니다 — 서버는 조각 파일이나 암호 문구를 절대 보지 못합니다.", "すべてブラウザ内で実行されます — サーバーがシェアやパスフレーズを見ることはありません。", "全程喺你嘅瀏覽器入面執行 — 伺服器永遠見唔到你嘅份額同密語。", "Vše běží ve vašem prohlížeči — server vaše podíly ani přístupovou frázi nikdy nevidí.", "Todo se ejecuta en tu navegador: ningún servidor ve jamás tus partes ni tu frase de contraseña.", "一切都在你的浏览器中运行——服务器永远不会看到你的份额或口令。", "Alles läuft in deinem Browser — kein Server sieht jemals deine Anteile oder Passphrase."),
    ...
    (tss_files_hint, "Pick any m of your n share files — whichever ones you still have.", "n개의 조각 파일 중 아무 m개나 선택하세요. 남아 있는 것이면 됩니다.", "n個のシェアファイルのうち任意のm個を選んでください。手元に残っているもので構いません。", "喺 n 個份額檔案入面揀任意 m 個 — 剩返邊啲用邊啲。", "Vyberte libovolných m z vašich n souborů s podíly — kterékoli, které ještě máte.", "Elige m cualesquiera de tus n archivos de partes: los que aún conserves.", "从 n 个份额文件中任选 m 个——手上还有哪些就用哪些。", "Wähle beliebige m deiner n Anteilsdateien — welche auch immer du noch hast."),
    (tss_files_loaded, "{have} of {need} required shares loaded — ready to sign in.", "필요한 조각 {need}개 중 {have}개 로드됨 — 로그인할 수 있습니다.", "必要なシェア{need}個のうち{have}個を読み込み済み — サインインできます。", "已載入 {have}/{need} 個所需份額 — 可以登入喇。", "Načteno {have} z {need} potřebných podílů — připraveno k přihlášení.", "Cargadas {have} de {need} partes necesarias: listo para iniciar sesión.", "已载入 {have}/{need} 个所需份额——可以登录了。", "{have} von {need} benötigten Anteilen geladen — bereit zur Anmeldung."),
    (tss_need_more, "Add {more} more share file(s) to reach the threshold.", "임계값에 도달하려면 조각 파일 {more}개를 더 추가하세요.", "しきい値に達するにはシェアファイルをあと{more}個追加してください。", "仲差 {more} 個份額檔案先夠門檻。", "Přidejte ještě {more} soubor(ů) s podíly, abyste dosáhli prahu.", "Añade {more} archivo(s) de partes más para alcanzar el umbral.", "还需再添加 {more} 个份额文件才能达到门限。", "Füge noch {more} Anteilsdatei(en) hinzu, um den Schwellwert zu erreichen."),
    (tss_files_mismatch, "These files belong to different wallets.", "이 파일들은 서로 다른 지갑의 것입니다.", "これらのファイルは別々のウォレットのものです。", "呢啲檔案屬於唔同嘅錢包。", "Tyto soubory patří různým peněženkám.", "Estos archivos pertenecen a carteras distintas.", "这些文件属于不同的钱包。", "Diese Dateien gehören zu verschiedenen Wallets."),
    (tss_files_invalid, "{name} is not an MPC share file.", "{name}은(는) MPC 조각 파일이 아닙니다.", "{name}はMPCシェアファイルではありません。", "{name} 唔係 MPC 份額檔案。", "{name} není soubor s MPC podílem.", "{name} no es un archivo de parte MPC.", "{name} 不是 MPC 份额文件。", "{name} ist keine MPC-Anteilsdatei."),
    (tss_files_legacy, "{name} is from an older version — its wallet can't sign here. Create a new MPC wallet and move the funds.", "{name}은(는) 이전 버전에서 만든 파일이라 여기서는 서명할 수 없습니다. 새 MPC 지갑을 만들어 자금을 옮기세요.", "{name}は古いバージョンで作られており、このウォレットでは署名できません。新しいMPCウォレットを作成して資金を移してください。", "{name} 係舊版本整嘅，喺呢度簽唔到名。開個新嘅 MPC 錢包再過錢啦。", "{name} pochází ze starší verze — jeho peněženka zde nemůže podepisovat. Vytvořte novou MPC peněženku a převeďte prostředky.", "{name} es de una versión anterior: su cartera no puede firmar aquí. Crea una cartera MPC nueva y mueve los fondos.", "{name} 来自旧版本——它的钱包无法在此签名。请创建新的 MPC 钱包并转移资金。", "{name} stammt aus einer älteren Version — diese Wallet kann hier nicht signieren. Erstelle eine neue MPC-Wallet und übertrage das Guthaben."),
    (tss_shape_hint, "Any {m} of the {n} shares can sign. You can lose up to {spare} of them and lose nothing.", "{n}개 조각 중 아무 {m}개로 서명할 수 있습니다. 최대 {spare}개까지 잃어도 아무것도 잃지 않습니다.", "{n}個のシェアのうち任意の{m}個で署名できます。最大{spare}個まで失っても何も失われません。", "{n} 個份額入面任意 {m} 個就簽到名。最多唔見咗 {spare} 個都唔會有損失。", "Podepsat může libovolných {m} z {n} podílů. Až {spare} z nich můžete ztratit, aniž byste o cokoli přišli.", "Cualquier combinación de {m} de las {n} partes puede firmar. Puedes perder hasta {spare} sin perder nada.", "{n} 个份额中任意 {m} 个即可签名。最多丢失 {spare} 个也不会有任何损失。", "Beliebige {m} der {n} Anteile können signieren. Du kannst bis zu {spare} davon verlieren, ohne etwas zu verlieren."),
    (tss_creating_hint, "Runs entirely in this browser — it takes just a few seconds.", "전부 이 브라우저 안에서 실행됩니다. 몇 초면 끝납니다.", "すべてこのブラウザ内で実行されます。数秒で終わります。", "全程喺呢個瀏覽器入面執行 — 幾秒就搞掂。", "Běží celé v tomto prohlížeči — trvá to jen pár sekund.", "Se ejecuta por completo en este navegador: tarda solo unos segundos.", "完全在此浏览器中运行——只需几秒钟。", "Läuft vollständig in diesem Browser — dauert nur wenige Sekunden."),
    (tss_backup_title, "Store your shares now", "지금 조각을 보관하세요", "今すぐシェアを保管してください", "而家就收好啲份額", "Uložte si nyní své podíly", "Guarda tus partes ahora", "现在就保存你的份额", "Sichere jetzt deine Anteile"),
    (tss_backup_intro, "Each file below is one share of this wallet. Store them in different places. Any {m} of them sign; fewer than {m} can do nothing; lose more than {spare} and the wallet is gone — there is no recovery.", "아래 각 파일은 이 지갑의 조각 하나입니다. 서로 다른 곳에 보관하세요. 아무 {m}개로 서명할 수 있고, {m}개 미만으로는 아무것도 할 수 없으며, {spare}개를 초과해 잃으면 지갑은 사라집니다 — 복구는 없습니다.", "下の各ファイルはこのウォレットのシェア1個です。別々の場所に保管してください。任意の{m}個で署名でき、{m}個未満では何もできません。{spare}個を超えて失うとウォレットは失われます — 復元はできません。", "下面每個檔案係呢個錢包嘅一個份額。放喺唔同嘅地方。任意 {m} 個簽到名；唔夠 {m} 個乜都做唔到；唔見多過 {spare} 個，個錢包就冇咗 — 冇得復原。", "Každý soubor níže je jeden podíl této peněženky. Uložte je na různá místa. Libovolných {m} podepíše; méně než {m} nezmůže nic; ztratíte-li více než {spare}, peněženka je pryč — obnova neexistuje.", "Cada archivo de abajo es una parte de esta cartera. Guárdalos en lugares distintos. Cualesquiera {m} firman; menos de {m} no pueden hacer nada; pierde más de {spare} y la cartera desaparece — no hay recuperación.", "下面每个文件是这个钱包的一个份额。请分开存放。任意 {m} 个可以签名；不足 {m} 个什么也做不了；丢失超过 {spare} 个钱包就没了——无法恢复。", "Jede Datei unten ist ein Anteil dieser Wallet. Bewahre sie an verschiedenen Orten auf. Beliebige {m} signieren; weniger als {m} können nichts; verlierst du mehr als {spare}, ist die Wallet weg — es gibt keine Wiederherstellung."),
    (tss_wallet_address, "Wallet address", "지갑 주소", "ウォレットアドレス", "錢包地址", "Adresa peněženky", "Dirección de la cartera", "钱包地址", "Wallet-Adresse"),
    (tss_share_n, "Share {i} of {n}", "조각 {i}/{n}", "シェア {i}/{n}", "份額 {i}/{n}", "Podíl {i} z {n}", "Parte {i} de {n}", "份额 {i}/{n}", "Anteil {i} von {n}"),
    (tss_download, "Download", "다운로드", "ダウンロード", "下載", "Stáhnout", "Descargar", "下载", "Herunterladen"),
    (tss_backup_gate, "Download every share to continue.", "계속하려면 모든 조각을 다운로드하세요.", "続行するにはすべてのシェアをダウンロードしてください。", "下載晒所有份額先可以繼續。", "Pokračujte až po stažení všech podílů.", "Descarga todas las partes para continuar.", "下载全部份额后才能继续。", "Lade jeden Anteil herunter, um fortzufahren."),
    (tss_signin_now, "Sign in with this wallet", "이 지갑으로 로그인", "このウォレットでサインイン", "用呢個錢包登入", "Přihlásit se touto peněženkou", "Iniciar sesión con esta cartera", "用这个钱包登录", "Mit dieser Wallet anmelden"),
    // The per-signature quorum prompt (dialogs/tss_quorum.rs): a TSS
    // session keeps no shares in memory and none in storage, so every
    // transaction asks for the files again.
    (tss_sign_title, "Sign with your share files", "조각 파일로 서명", "シェアファイルで署名", "用份額檔案簽名", "Podepsat soubory s podíly", "Firmar con tus archivos de partes", "用份额文件签名", "Mit Anteilsdateien signieren"),
    (tss_sign_body, "This wallet signs by ceremony. Present enough of its share files and the passphrase — they are used once, in this browser, and never stored.", "이 지갑은 서명 의식으로 서명합니다. 조각 파일을 충분히 제시하고 암호 문구를 입력하세요. 이 브라우저 안에서 한 번만 사용되며 절대 저장되지 않습니다.", "このウォレットはセレモニーで署名します。シェアファイルを必要な数だけ提示し、パスフレーズを入力してください。このブラウザ内で一度だけ使われ、保存されることはありません。", "呢個錢包靠儀式簽名。提供足夠嘅份額檔案同密語 — 只會喺呢個瀏覽器入面用一次，唔會儲存。", "Tato peněženka podepisuje ceremoniálem. Předložte dostatek jejích souborů s podíly a přístupovou frázi — použijí se jednou, v tomto prohlížeči, a nikdy se neukládají.", "Esta cartera firma por ceremonia. Presenta suficientes archivos de partes y la frase de contraseña: se usan una vez, en este navegador, y nunca se guardan.", "此钱包通过仪式签名。请提供足够的份额文件和口令——仅在此浏览器中使用一次，绝不存储。", "Diese Wallet signiert per Zeremonie. Lege genügend ihrer Anteilsdateien und die Passphrase vor — sie werden einmal, in diesem Browser, verwendet und nie gespeichert."),
    (tss_sign_button, "Sign", "서명", "署名", "簽名", "Podepsat", "Firmar", "签名", "Signieren"),
    (tss_sign_hint, "Used for this signature only, then forgotten.", "이번 서명에만 사용된 뒤 즉시 지워집니다.", "この署名にだけ使われ、その後は破棄されます。", "只用喺呢次簽名，之後即刻唔記得。", "Použije se jen pro tento podpis a pak se zapomene.", "Se usa solo para esta firma y luego se olvida.", "仅用于本次签名，随后即被丢弃。", "Nur für diese Signatur verwendet, danach verworfen."),
    (tss_wrong_wallet, "These share files belong to a different wallet.", "이 조각 파일은 다른 지갑의 것입니다.", "このシェアファイルは別のウォレットのものです。", "呢啲份額檔案係屬於另一個錢包嘅。", "Tyto soubory s podíly patří jiné peněžence.", "Estos archivos de partes pertenecen a otra cartera.", "这些份额文件属于另一个钱包。", "Diese Anteilsdateien gehören zu einer anderen Wallet."),
    (tss_local_unavailable, "MPC wallets need a server — they aren't available in local mode.", "MPC 지갑은 서버가 필요하므로 로컬 모드에서는 사용할 수 없습니다.", "MPCウォレットにはサーバーが必要なため、ローカルモードでは使えません。", "MPC 錢包需要伺服器，本機模式用唔到。", "MPC peněženky potřebují server — v lokálním režimu nejsou dostupné.", "Las carteras MPC necesitan un servidor: no están disponibles en modo local.", "MPC 钱包需要服务器——本地模式下不可用。", "MPC-Wallets brauchen einen Server — im lokalen Modus nicht verfügbar."),
}
...
mod tests {
    ...
    fn nothing_is_left_in_english_by_accident() {
        ...
        let shared_by_design: &[(Lang, Key)] = &[
            ...
            // "Passphrase" is the ordinary German noun too, same rule as
            // Key::wallet above.
